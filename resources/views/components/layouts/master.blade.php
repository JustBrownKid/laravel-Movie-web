...
<head>
    ...
    <title>@yield('title', 'Feel like God')</title>
...
<body class="bg-gray-100 h-screen overflow-hidden">
    ...
    <div class="flex h-[calc(100vh-64px)] lg:h-full">
                ...
                <nav class="flex-1 flex flex-col space-y-2 px-4 mt-4 overflow-y-auto">
                    <div x-data="{ open: false }" class="relative inline-block text-left w-full">
                        ...
                        <button @click="open = !open"
                            class="flex items-center gap-2 px-4 py-3 w-full border-b border-gray-200 dark:border-gray-700 text-left hover:bg-gray-400 dark:hover:bg-gray-800 transition">
                            ...
                            <span class="material-icons text-gray-500 transition-transform duration-300"
                        ...
                        <div x-show="open" @click.outside="open = false" x-transition
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit"
                                    class="flex items-center gap-2 text-red-600 hover:text-red-200 transition">
                    ...
                    @if (auth()->user()->role == 'admin')
                        <a href="{{ route('admin.penal') }}"
                            class="flex items-center px-3 py-2 rounded hover:bg-gray-700 text-white">
                            <span class="material-icons mr-3">admin_panel_settings</span>Admin Dashboard    
                        </a>
                    @endif

                    <a href="{{ route('dashboard') }}"
                        class="flex items-center px-3 py-2 rounded hover:bg-gray-700 text-white">
                        <span class="material-icons mr-3">dashboard</span> Home Page
                    </a>
        ...
        <!-- Main Content -->
        <main class="flex-1 m-0 p-0 bg-zinc-400  transition-all duration-300 overflow-y-auto h-screen">
            
<div class="max-w-7xl mx-auto  ">
<div class="bg-gray-900 p-8 -xl shadow-xl border border-gray-800 max-w-full mx-auto">
            @yield('content')   
</div>
</div>
        </main>
